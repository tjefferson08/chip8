...
(defn run-program [p]
  (sut/run (sut/init (emu/init-ram p))))
...
(deftest load-complex
  (let [ctx (run-program [0x63 0x0E ;; LD F, 0x0E
                          0xF3 0x29 ;; LD F, V3 (load E sprite address)
                          0x00 0xFD])]  ;; EXIT
    (is (= 0x46 (sut/read-reg ctx :i)))) ;; 0x0E * 5

  (let [ctx (run-program [0x64 0xAD ;; LD V4, 0xAD (173)
  ...
  (let [ctx (run-program [0x60 0xFF ;; LD V0, 0xFF
...
(deftest skip
  (let [ctx (run-program [0x60 0xAF ;; LD V0, 0xAF
